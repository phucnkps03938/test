...
Public Class Class1
    Public Function Loadkhachang() As DataSet
        ...
        Dim chuoiketnoi As String = "server=(local);database=SANPHAM;trusted_connection=false;uid=sa;pwd=666999"
    ...
    Public Function Loadsanpham() As DataSet
        ...
        Dim chuoiketnoi As String = "server=(local);database=SANPHAM;trusted_connection=false;uid=sa;pwd=666999"
        Dim conn As SqlConnection = New SqlConnection(chuoiketnoi)
        Dim LoadSP As New SqlDataAdapter("select SANPHAM1.MASP as 'Mã sản phẩm',SANPHAM1.TENSP as 'Tên sản phẩm', LOAISANPHAM.MALOAI as 'Mã Loại', LOAISANPHAM.TENLOAI as 'Tên Loại',SANPHAM.SOLUONG as 'Số lượng' from SANPHAM inner join LOAISANPHAM on SANPHAM.MASP = LOAISANPHAM.MASP", conn)
        Dim db As New DataSet
        conn.Open()
        LoadSP.Fill(db)
        conn.Close()
